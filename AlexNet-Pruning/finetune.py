...
class ModifiedAlexNetModel(torch.nn.Module):
	def __init__(self):
		...
		print("After getting the model")
		self.features = model.features
		#print("This is the type: ",model.features.type)
		for param in self.features.parameters():
			param.requires_grad = False # Setting the gradient requirement as false ...why?
...
class PrunningFineTuner_AlexNet:
	...
	def train(self, optimizer = None, epoches = 10): # This is the function used for training...
	...
	def train_batch(self, optimizer, batch, label, rank_filters):  # This is the function used for training...
	...
	def train_epoch(self, optimizer = None, rank_filters = False): # This is the function used for training...
	...
	def prune(self):
		...
		number_of_filters = self.total_num_filters() # Calculating the total number of filters...
		num_filters_to_prune_per_iteration = 64
		iterations = int(float(number_of_filters) / num_filters_to_prune_per_iteration)

		iterations = int(iterations * 2.0 / 3)
		print ("Number of prunning iterations to reduce", 300/4,"% filters", iterations)
...
def test_single(model,test_single_path):
	test_single_loader_ = dataset.test_single_loader(test_single_path)
	model.eval()
	time_ = 0 
	for i, (batch, label) in enumerate(test_single_loader_):
		t0 = time.time()
		output = model(Variable(batch))
		t1 = time.time()
		print t1-t0
		if i!=0:
			time_ +=t1-t0
		pred = output.data.max(1)[1]
	print ("Avg time taken per image is(over 800 Images..)  ", time_/799)
